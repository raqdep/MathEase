...
function rfUpdateLessonCompletionStatus() {
    ...
    buttons.forEach(btn => {
        ...
        btn.classList.toggle('completed', rfCompletedLessons.has(lesson));
        const icon = btn.querySelector('.w-16');
        if (icon) {
            if (rfCompletedLessons.has(lesson)) {
...
async function rfCompleteLesson(lessonNum) {
    try {
        ...
        if (rfCompletedLessons.has(lessonNum)) {
            rfSetCompleteButtonState(lessonNum, { completed: true });
            return;
        }

        rfSetCompleteButtonState(lessonNum, { loading: true });

        const response = await fetch('../php/complete-lesson.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ topic: 'rational-functions', lesson: lessonNum, action: 'complete' }),
            credentials: 'include'
        });
        ...
        if (data && data.success) {
            ...
            rfSetCompleteButtonState(lessonNum, { completed: true });
            // Reload from server to ensure state is in sync
            await rfLoadCompletedLessons();
            rfUpdateLessonCompletionStatus();
        } else {
            rfSetCompleteButtonState(lessonNum, { completed: false });
...
async function rfLoadCompletedLessons() {
    try {
        // Prefer precise list from complete-lesson endpoint
        const res = await fetch('../php/complete-lesson.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'get_completed', topic: 'rational-functions' }),
            credentials: 'include',
            cache: 'no-store'
        });
        if (res.ok) {
            ...
            rfCompletedLessons = new Set(list);
            rfUpdateLessonCompletionStatus();
            return;
        }
        // Fallback to general progress API (may only have counts)
        const fallback = await fetch('../php/get-progress.php', { credentials: 'include', cache: 'no-store' });
        if (fallback.ok) {
            const data2 = await fallback.json();
            const topicKey = 'rational-functions';
            const count = (data2 && data2.details && data2.details[topicKey] && data2.details[topicKey].lessons_completed) || 0;
            const approx = Array.from({ length: Math.max(0, Math.min(count, rfTotalLessons)) }, (_, i) => i + 1);
            rfCompletedLessons = new Set(approx);
            rfUpdateLessonCompletionStatus();
        }
    } catch (_) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    lessonNavBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // Update current lesson and UI
            rfCurrentLesson = parseInt(lessonNum, 10);
            rfUpdateNavigationButtons();
            rfUpdateProgressIndicators();
            rfUpdateLessonCompletionStatus();
    ...
    // Inject completion + nav controls and initialize state
    try { rfInjectLessonControls(); } catch (_) {}
    lessonNavBtns[0].click();
    // Initialize completion buttons state on first load
    rfUpdateCompletionButtonsUI();
    // Try loading previously completed lessons (best-effort)
    try { rfLoadCompletedLessons(); } catch (_) {}
