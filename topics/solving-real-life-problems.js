...
async function completeLesson(lessonNum) {
    try {
        ...
        const response = await fetch('../php/complete-lesson.php', {
            ...
            body: JSON.stringify({ topic: 'solving-real-life-problems', lesson: lessonNum, action: 'complete' }),
            credentials: 'include'
        });
...
async function slpLoadCompletedLessons() {
    try {
        const response = await fetch('../php/complete-lesson.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ topic: 'solving-real-life-problems', action: 'get_completed' }),
            credentials: 'include'
        });
        if (response.ok) {
            const data = await response.json();
