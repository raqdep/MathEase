...
async function rprfCompleteLesson(lessonNum) {
    try {
        ...
        const response = await fetch('../php/complete-lesson.php', {
            ...
            body: JSON.stringify({ topic: 'representations-of-rational-functions', lesson: lessonNum, action: 'complete' }),
            credentials: 'include'
        });
        ...
        if (data && data.success) {
            // Reload authoritative completion from server
...
async function rprfLoadCompletedLessons() {
    try {
        ...
        const res = await fetch('../php/complete-lesson.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'get_completed', topic: 'representations-of-rational-functions' }),
            credentials: 'include',
            cache: 'no-store'
        });
        if (res.ok) {
