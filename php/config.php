...
define('DB_NAME', getenv('DB_NAME') ?: 'mathease_db');
...
function send_working_smtp($to, $subject, $htmlBody) {
    ...
    try {
        ...
        // Start TLS - Gmail requires this
        fputs($socket, "STARTTLS\r\n");
        $response = fgets($socket, 1024);
        // Gmail sometimes returns 250 instead of 220 for STARTTLS
...
function is_teacher_logged_in() {
    // Consider a user as a teacher if either a teacher_id is present in session
    // or the session explicitly marks user_type as 'teacher'.
    // This makes the check resilient to sessions that only set one of these.
