...
function student_audit_action_labels_en(): array
{
    return [
        'login'                     => 'Signed in',
        'logout'                    => 'Signed out',
        'class_join_request'        => 'Requested to join a class',
        'class_join_approved'       => 'Class enrollment approved',
        'class_join_pending'        => 'Awaiting class approval',
        'quiz_completed'            => 'Completed a quiz',
        'quiz_ended'                => 'Quiz session ended',
        'lesson_progress'           => 'Lesson progress',
        'flashcards_open'           => 'Opened Flashcards',
        'achievements_open'         => 'Visited Achievements',
        'profile_edit'              => 'Edited profile',
        'password_change'           => 'Changed password',
        'lesson_quiz_answer'        => 'Answered lesson quiz',
        'achievement_earned'        => 'Earned an achievement',
        'last_session'              => 'Last recorded sign-in (account)',
        'teacher_lesson_view'       => 'Viewed teacher lesson',
    ];
}
 ...
 *
 * @return list<array{action:string,label:string,label_tl:string,details:?string,at:string,source:string}>
 */
function build_student_audit_trail_for_teacher(PDO $pdo, int $teacherId, int $studentId, int $limit = 150): array
{
    ...
    ensure_student_activity_log_table($pdo);
    $labels = student_audit_action_labels_en();
    $rows = [];
    ...
    $add = function (string $action, ?string $details, string $at, string $source) use (&$rows, $labels) {
        ...
        $lbl = $labels[$action] ?? ucfirst(str_replace('_', ' ', $action));
        $rows[] = [
            'action'   => $action,
            'label'    => $lbl,
            'label_tl' => $lbl,
